好的，以下是 **sqli-labs Less-2（数字型联合注入）** 的完整 Markdown 报告，格式与 Less-1 一致，只需粘贴图片即可。
...
> 📌 **ORDER BY**：SQL中用来按第几列排序的命令。`ORDER BY 4` 表示按第4列排序，若原表只有3列，则报错。通过这个机制可以推断字段数量。
...
```
页面回显数字 **2** 和 **3**，说明第2、3个字段会被显示在页面中，可用于放置要窃取的数据。
...
> 📌 **database()**：MySQL 内置函数，返回当前连接的数据库名称。在联合注入中常放在回显位上直接获取库名。
...
> 📌 **新函数讲解 —— group_concat()**
> `group_concat(列名)` 是 MySQL 的聚合函数，将同一列的多行值拼接成一个字符串，默认用逗号分隔。在 SQL 注入中用于一次性获取某个表的所有表名、列名或数据，避免逐条查询。

> 📌 **information_schema.tables**：MySQL 自带的元数据表，记录了所有数据库的所有表信息。`table_schema` 字段指定数据库名，`table_name` 字段为表名。
...
> 📌 **information_schema.columns**：MySQL 自带的元数据表，记录了所有数据库中所有表的列信息。`table_name` 字段指定表名，`column_name` 字段为列名。

![获取列名](step6.png)

### 4.7 获取数据（脱库）

查询 `users` 表中的用户名和密码，用冒号分隔：
```
...
-- 3. database() 返回的库名被显示在回显位2
-- 4. --+ 注释掉后面的 LIMIT 0,1 等语句，避免干扰

-- 与Less-1（字符型）的关键区别：
-- Less-1：WHERE id='$id' → 需要先用 ' 闭合引号
-- Less-2：WHERE id=$id  → 参数直接拼接，无需闭合
```
...
1. 进入 `SQL-Injection/` 文件夹
2. 点 `Add file` → `Create new file`
3. 文件名输入：`sqli-labs-Less2/README.md`
4. 粘贴上面全部内容
5. 提交
6. 上传截图到 `sqli-labs-Less2/` 文件夹（共7张：step1.png ~ step7.png）

搞完告诉我，继续给你 Less-5 的完整报告。
