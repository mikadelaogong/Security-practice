...
![php-reverse-shell脚本](image24.png)
...
![反弹Shell成功](image25.png)

![Shell连接确认](image26.png)
...
我明白了，你要的是可以直接粘贴到文件中、不会显示为代码块的整体内容。我会注意在输出时不在最外层包裹代码块，让你能直接作为 Markdown 正文使用。

```markdown
### 3.25 升级为交互式 Shell

当前获取的 Shell 为非交互式，功能受限。使用 Python 命令将其升级为完整的交互式 TTY Shell：

```bash
python -c "import pty;pty.spawn('/bin/bash')"
```

![Python升级Shell](image27.png)

### 3.26 内网信息收集与提权

查看 `/home/` 目录，发现目标系统上存在一个名为 **togie** 的用户（与之前在 WordPress 前台看到的用户名一致）。

![查看home目录](image28.png)

通过进一步的信息收集和权限分析，找到本地提权途径，成功将权限从 www-data 提升至 **root**，完全控制目标服务器。
...
- Kali Linux 工具文档: https://www.kaali.org/tools/
```
